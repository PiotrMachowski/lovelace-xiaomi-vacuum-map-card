...
export class XiaomiVacuumMapCard extends LitElement {
    ...
    private _isInEditor(): boolean {
        return this.parentElement?.tagName?.toLowerCase() === "hui-card-preview";
    }

    private _handleAutogeneratedConfigGet(): void {
        const event = new Event(EVENT_AUTOGENERATED_CONFIG);
        (event as any).presetConfig = {
            ...(this.currentPreset ?? {}),
            map_modes: this.modes.map(m => m.toMapModeConfig()),
        };
        this._getRoomsConfig();
        window.dispatchEvent(event);
    }
